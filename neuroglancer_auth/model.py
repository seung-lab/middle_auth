...
class Group(db.Model):
    ...
    def get_users(self):
        query = db.session.query(UserGroup.user_id, User.username)\
...
class GroupDataset(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    group_id = db.Column('group_id', db.Integer, db.ForeignKey("group.id"), nullable=False)
    dataset_name = db.Column('dataset_id', db.String(32), nullable=True, index=True) # nullable because the dataset may be deleted and we want to represent that without deleting the row
