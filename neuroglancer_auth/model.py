...
class User(db.Model):
    ...
    name = db.Column(db.String(80, collation='NOCASE'), unique=False, nullable=False)
...
class Group(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    name = db.Column(db.String(120, collation='NOCASE'), unique=True, nullable=False)
...
class Dataset(db.Model):
    ...
    def search_by_name(name):
        if name:
            return Dataset.query.filter(Dataset.name.like(f'%{name}%')).all()
        else:
