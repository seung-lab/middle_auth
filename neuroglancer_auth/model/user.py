...
from .api_key import insert_and_generate_unique_token, APIKey, delete_token, tokens_key, delete_all_tokens_for_user
...
class User(db.Model):
    ...
    def datasets_missing_tos(self):
        ...
        query = db.session.query(Dataset.id, Dataset.name, Tos.id, Tos.name)\
            ...
            .join(UserTos, (UserTos.tos_id == Tos.id) & (UserTos.user_id == self.id), isouter=True)\
