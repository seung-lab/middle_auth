...
class GroupDatasetPermission(db.Model):
    ...
    def add(group_id, dataset_id, permission_ids=[]):
        print("GroupDatasetPermission.add")
        for permission_id in permission_ids:
            print(f"GroupDatasetPermission.add addding {permission_id}")
            gd = GroupDatasetPermission(group_id=group_id, dataset_id=dataset_id, permission_id=permission_id)
            db.session.add(gd)
        db.session.commit()
    ...
    def get_permissions_for_group(group_id):
        ...
        return [{'id': dataset_id, 'name': dataset_name, 'permission': permission_name, 'permission_id': permission_id} for dataset_id, dataset_name, permission_name, permission_id in permissions]

    @staticmethod
    def get_all_group_permissions(dataset_id):
        query = db.session.query(GroupDatasetPermission.group_id, Group.name, Permission.name, Permission.id)\
        ...
        return [{'id': group_id, 'name': group_name, 'permission': permission_name, 'permission_id': permission_id} for group_id, group_name, permission_name, permission_id in permissions]
