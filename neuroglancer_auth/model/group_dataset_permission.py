...
class GroupDatasetPermission(db.Model):
    ...
    @staticmethod
    def add(group_id, dataset_id, permisson_id):
        gd = GroupDatasetPermission(group_id=group_id, dataset_id=dataset_id, level=level)
        db.session.add(gd)
        db.session.commit()
    ...
    def get_permissions_for_group(group_id):
        ...
        return [{'id': dataset_id, 'dataset': dataset_name, 'permission': permission_name} for dataset_id, dataset_name, permission_name in permissions]
    ...
    def get_all_group_permissions(dataset_id):
        ...
        return [{'id': group_id, 'dataset': group_name, 'permission': level} for group_id, group_name, level in permissions]
