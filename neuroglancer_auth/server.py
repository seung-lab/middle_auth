...
@api_v1_bp.route(f'/{"boop"}/<name>', methods=['GET'])
@auth_required
def boopboop(name):
    return flask.render_template('tos.html', name=name)

@api_v1_bp.route(f'/{"boop"}/<name>', methods=['POST'])
@auth_required
def boopbooppost(name):
    return flask.jsonify(f"thanks you completed {name}")
