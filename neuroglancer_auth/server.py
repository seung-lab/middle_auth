...
def maybe_handle_tos(user):
    ...
    if tos_id:
        return redirect_with_args(flask.url_for('api_v1_bp.tos_accept_view', tos_id=tos_id), ['middle_auth_token'])
    else:
...
def oauth2callback():
    ...
    if user is None:
        ...
        return redirect_with_args(flask.url_for('api_v1_bp.register_choose_username_view'), ['middle_auth_token'])
    else:
