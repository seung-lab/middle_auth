...
def create_user_route():
    ...
    try:
        user = User.create_account(data['email'], data['name'], data['pi'], False, False, group_names=["default"])
...
def my_before_request():
    print("hello! tbr requiring admin")


@br_bp.route('/admincheck')
def tbrone():
    return flask.jsonify("you must be an admin!")


@auth_requires_permission("view")
@br_bp.route('/<table_id>/testbr')
def tbrtwo(table_id):
    return flask.jsonify("tbr requiring view 2!")

@auth_requires_admin
@br_bp.route('/doubleadmin')
def doubleadmintest(table_id):
    return flask.jsonify("finished doubleadmin")
