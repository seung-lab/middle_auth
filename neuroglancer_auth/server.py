...
from .dec import auth_required, auth_requires_admin, auth_requires_roles, auth_required_table
import sqlalchemy
...
@mod.route('/user/<int:user_id>/groups')
@auth_requires_admin
...
@mod.route('/group/<int:group_id>/dataset', methods=['POST'])
@auth_requires_admin
def add_dataset_to_group_route(group_id):
    ...
    if data and 'dataset_id' in data:
        try:
            GroupDataset.add(group_id, int(data['dataset_id']), data.get('view', False), data.get('edit', False), data.get('admin', False))
            return flask.jsonify("success")
        except sqlalchemy.exc.IntegrityError as err:
...
@mod.route('/group/<int:group_id>/dataset/<int:dataset_id>', methods=['PUT'])
@auth_requires_admin
...
@mod.route('/group/<int:group_id>/dataset/<int:dataset_id>', methods=['DELETE'])
@auth_requires_admin
...
@mod.route('/testtable/<table_id>')
@auth_required_table
def test_api_request_table(table_id):
    return flask.jsonify({
        "a": table_id,
        "b": flask.g.test_table
    })
