...
def authorize():
    ...
    flask.session['origin'] = flask.request.args.get('origin')

    resp = flask.Response(authorization_url)
    resp.headers['Access-Control-Allow-Origin'] = flask.session['origin']
    resp.headers['Access-Control-Allow-Credentials'] = 'true'
    return resp

@mod.route("/version")
def version():
    return "neuroglance_auth -- version a"
...
def oauth2callback():
    ...
    return flask.redirect(flask.session['origin'] + '?token=' + our_token, code=302)
