...
def register_choose_username_post():
    ...
    if form_custom:
        ...
        if same_custom or UserCustomName.add(user.id, form_custom):
            return maybe_handle_tos(user, flask.g.auth_token, 'msg.html', {"title": "User Settings Updated", "msg": f"Your username was changed to {form_custom}"})
        else:
        ...
        return maybe_handle_tos(user, flask.g.auth_token, 'msg.html', {"title": "User Settings Updated", "msg": "Your username was reset to the one associated with your Google Account"})
...
def tos_accept_view(tos_id):
    ...
    if existing:
        return flask.render_template('msg.html', title=f"{tos.name}'s Terms of Service", msg="You have already accepted the Terms of Service")
    else:
