...
def finish_auth_flow(user):
    apps = [app.as_dict() for app in App.get_all()]
    def create_auth_flow_end_redirect(token):
        return f"""<script type="text/javascript">
            if (window.opener) {{
                window.opener.postMessage({{token: "{token}", apps: {apps}}}, "*");
            }}
            </script>"""
...
@auth_required
def get_apps():
    return flask.jsonify([app.as_dict() for app in App.get_all()])
