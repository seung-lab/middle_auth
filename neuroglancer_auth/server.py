...
from .decs import auth_required, auth_requires_roles
import sqlalchemy
...
def authorize():
    ...
    print("redirect 1: {0}".format(flask.session['redirect']))
...
def oauth2callback():
    ...
    print("redirect 2: {0}".format(flask.session['redirect']))

    print("furl 1: {0}".format(furl(flask.session['redirect']).add({'token': token}).url))
