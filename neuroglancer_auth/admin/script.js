...
async function refreshSelectedUser() {
	...
	const userInfo = await authFetch(`${AUTH_URL}/user/${selectedUserId}`).then((res) => {
		return res.json();
	});

	const usersGroups = await authFetch(`${AUTH_URL}/user/${selectedUserId}/groups`).then((res) => {
		return res.json();
	});

	userInfo.groups = usersGroups;

	renderItem(userInfo, document.getElementById('otherUserData'));
