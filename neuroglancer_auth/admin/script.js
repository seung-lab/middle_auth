...
const datasetDataApp = {
	...
	methods: {
		async updateAvailableGroups() {
			this.groups = await authFetch(`${AUTH_URL}/dataset/${this.dataset.id}/groups`);
		...
		async addGroupDataset() {
			await authFetch(`${AUTH_URL}/group/${this.selectedGroup}/dataset`, {
				method: 'POST',
				headers: {
		...
		async removeGroup(group) {
			await authFetch(`${AUTH_URL}/group/${group.id}/dataset/${this.dataset.id}`, {
				method: 'DELETE'
			});
		...
		addAdmin() {
			...
			const users = res.authFetch(`${AUTH_URL}/group/${group.id}/dataset/${this.dataset.id}`)
		}
	},
	template: `
	<div>
