...
const datasetDataApp = {
	...
	<template v-else>
		<div id="userData">
			...
			<input v-model="dataset.name" placeholder="Name">
...
const groupDataApp = {
	...
	methods: {
		async addDataset() {
			console.log('addDataset', this.selectedDataset);
			...
			this.datasets = await authFetch(`${AUTH_URL}/dataset`);
		},
		async updatePermissions(dataset_id) {
			console.log('update dataset', dataset_id);
		},
		async removeDataset(dataset_id) {
			console.log('remove dataset', dataset_id);
		}
	},
	template: `
	<div>
			...
			<div class="listContainer">
				...
				<div class="datasets list" data-link="dataset">
					<div v-for="dataset in datasets">
						...
						<template v-for="(pon, pname) in dataset.permission">
							<div><input type="checkbox" v-model="dataset.permission[pname]" @change="updatePermissions(dataset.id)"></div>
						</template>
...
const listApp = {
	...
	<router-link :to="{ path: 'create' }" append>Create</router-link>
