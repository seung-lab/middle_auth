...
@cachetools.func.ttl_cache(maxsize=CACHE_MAXSIZE, ttl=CACHE_TTL)
def cell_is_public(table_id, cell_id, token):
    print("cell_is_public")
    print("token: " + token)
    if cell_id is None:
        return False
    
    cip_url = 'https://{}/api/v1/table/{}/cell/{}/is_public'.format(AUTH_URL, table_id, cell_id)

    print("cip_url: " + cip_url)

    user_request = requests.get(cip_url, headers={'authorization': 'Bearer ' + token})
    ...
    def decorator(f):
        ...
        def decorated_function(table_id, *args, **kwargs):
            ...
            if dataset is not None:
                ...
                if has_permission or cell_is_public(table_id, kwargs.get('cell_id'), flask.g.auth_token):
                    return f(*args, **{**kwargs, **{'table_id': table_id}})
                else:
