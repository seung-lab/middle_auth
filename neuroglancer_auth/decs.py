    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            def lazy_check_public_access():
                global public_access_cache

                if public_access_cache is None:
                    if service_token and required_permission is not 'edit':
                        if public_node_key is not None:
                            flask.g.public_access = is_root_public(kwargs.get(public_table_key), kwargs.get(public_node_key), service_token)
                        elif public_table_key is not None:
                            flask.g.public_access = table_has_public(kwargs.get(public_table_key), service_token)
                        else:
