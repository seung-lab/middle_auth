...
def auth_required(func=None, *, required_permission=None, public_table_key=None, public_node_key=None):
    # if func is None:
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            flask.g.public_access = False
            if required_permission is not 'edit': 
                if public_node_key is not None:
                    flask.g.public_access = is_root_public(kwargs.get(public_table_key), kwargs.get(public_node_key))
                elif public_table_key is not None:
                    flask.g.public_access = is_root_public(kwargs.get(table_key))
...
def auth_requires_permission(required_permission, public_table_key, public_node_key):
    def decorator(f):
        @wraps(f)
        @auth_required(required_permission=required_permission, public_table_key=public_table_key, public_node_key=public_node_key)
